...
#define ADC_CHANNELS ADC_CHER_CH7
...
void ADCSampler::begin(unsigned int samplingRate)
{
  ...
  */
  ...
  ADC->ADC_PTCR  =  ADC_PTCR_RXTEN;  // Enable receiving data.   Transfer Control Register
...
void ADCSampler::handleInterrupt()
{
  ...
  //Serial.print("ADC\n");
    ...
    ADC->ADC_RNPR  = (unsigned long) adcBuffer[(adcDMAIndex + 1) % NUMBER_OF_BUFFERS];//Receive Next Pointer Register
    ADC->ADC_RNCR  = BUFFER_SIZE; //Receive Next Counter Register
    dataReady = true;
  }
}
...
//  SerialUSB.println("This is USB serial");
...
void loop() {
  yield();//把控制权交给loopADC，如果loop里没干别的相当于一只在运行loopADC
  //player.push();
//  SerialUSB.print(".");
}
